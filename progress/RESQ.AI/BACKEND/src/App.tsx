        ...
        {user ? (
          <div className="flex items-center gap-3">
            <div className="flex flex-col items-end hidden sm:flex">
              <span className="text-xs font-bold text-on-surface">{user.name || user.login}</span>
              <button onClick={onLogout} className="text-[10px] text-on-surface-variant hover:text-error transition-colors uppercase font-bold tracking-widest">Logout</button>
            </div>
            <img src={user.avatar_url} alt={user.login} className="w-10 h-10 rounded-full border-2 border-primary/20" referrerPolicy="no-referrer" />
          </div>
        ) : (
          <button 
            onClick={onLogin}
            className="bg-on-surface text-white px-6 py-2.5 rounded-xl font-semibold text-sm hover:scale-95 transition-transform duration-200 flex items-center gap-2"
          >
...
const Hero = ({ onCheckRisk, loading }: { onCheckRisk: (location: string) => void, loading: boolean }) => {
        ...
        <div className="glass-panel p-2 rounded-2xl fog-shadow flex flex-col md:flex-row items-center gap-2">
          <div className="flex-1 w-full flex items-center px-4 gap-3 bg-surface-container-highest/50 rounded-xl border border-transparent focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/10 transition-all">
            ...
            <input 
              ...
              onKeyPress={(e) => e.key === 'Enter' && onCheckRisk(location)}
            />
          </div>
          <button 
            disabled={loading}
            onClick={() => onCheckRisk(location)}
            className="w-full md:w-auto bg-gradient-to-br from-primary to-tertiary text-white px-10 py-4 rounded-xl font-bold tracking-tight hover:scale-[0.98] transition-transform flex items-center justify-center gap-2 disabled:opacity-50"
          >
...
const Dashboard = ({ riskData }: { riskData: RiskData | null }) => {
  const displayLocation = riskData?.location || "San Francisco, CA";
  const displayRisk = riskData?.risk || { type: "Flood Watch", level: "Medium Alert", probability: "64%", icon: "Waves" };

  return (
    <section className="mt-24 w-full max-w-7xl mx-auto px-6">
      <div className="mb-12">
        ...
        <div className="md:col-span-8 bg-surface-container-lowest rounded-3xl p-8 fog-shadow relative overflow-hidden group">
          ...
          <div className="relative z-10 h-full flex flex-col justify-between">
            <div>
              ...
              <h3 className="text-3xl font-headline font-bold text-primary mb-6">{displayLocation}</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mt-4">
                <div className="flex items-start gap-4 p-4 rounded-2xl bg-surface-container-low">
                  <div className="bg-primary-container p-3 rounded-xl text-primary">
                    {displayRisk.icon === 'Waves' ? <Waves className="w-6 h-6" /> : <ShieldAlert className="w-6 h-6" />}
                  </div>
                  <div>
                    <p className="text-xs font-label text-on-surface-variant">DISASTER TYPE</p>
                    <p className="font-headline font-bold text-lg">{displayRisk.type}</p>
                    <p className="text-sm text-on-surface-variant">Probability index: {displayRisk.probability}</p>
                  </div>
                </div>
                <div className="flex items-start gap-4 p-4 rounded-2xl bg-error-container/10">
                  ...
                  <div>
                    ...
                    <p className="font-headline font-bold text-lg text-error">{displayRisk.level}</p>
                    <p className="text-sm text-on-surface-variant">Monitoring active</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6 flex-grow">
            <div className="p-4 rounded-xl bg-tertiary-container/10 border-l-4 border-tertiary">
              <p className="text-sm italic leading-relaxed text-on-surface">
                "Atmospheric pressure shifts suggest a cooling trend by Thursday. Heatwave risk is diminishing, but soil saturation remains high."
              </p>
            </div>
...
const RiskOverview = ({ riskData }: { riskData: RiskData | null }) => {
  const stats = riskData?.stats || { heatwave: '72°F', humidity: '42%', wind: '12 mph' };

  return (
    <section className="mt-12 w-full max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-6">
      {[
        { icon: Thermometer, label: 'Heatwave Index', value: stats.heatwave, color: 'bg-green-500', risk: 'Low Risk', riskColor: 'text-green-700 bg-green-50' },
        { icon: Droplets, label: 'Humidity Impact', value: stats.humidity, color: 'bg-blue-500', risk: 'Monitoring', riskColor: 'text-blue-700 bg-blue-50' },
        { icon: Wind, label: 'Wind Speed', value: stats.wind, color: 'bg-primary', risk: 'Normal', riskColor: 'text-primary bg-primary-container/20' },
      ].map((item, i) => (
      ...
      <div className="space-y-6">
        ...
        <div className="bg-surface-container-highest p-8 rounded-3xl space-y-8">
          {[
            ...
            <div key={i} className="flex gap-4">
              <item.icon className="text-tertiary w-5 h-5 flex-shrink-0" />
              <div className="space-y-1">
                <p className="font-semibold text-on-surface">{item.title}</p>
                <p className="text-sm text-on-surface-variant">{item.desc}</p>
              </div>
...
export default function App() {
  ...
  const handleCheckRisk = async (location: string) => {
    if (!location) return;
    setLoading(true);
    try {
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="flex flex-col items-center">
        ...
        <Hero onCheckRisk={handleCheckRisk} loading={loading} />
        <Stats />
            ...
            <div className="md:col-span-7">
              ...
              >
                ...
                <div className="absolute bottom-6 left-6 text-white text-left">
                  ...
                  <h3 className="font-headline font-bold text-2xl">Atlantic Basin: Stabilized</h3>
                </div>
              </motion.div>
            </div>
            <div className="md:col-span-5 pb-12">
              ...
              >
                ...
                <p className="text-on-surface-variant leading-relaxed text-sm">
                  Our neural networks are currently processing 4.2TB of climate telemetry from the European Space Agency. No immediate high-risk anomalies detected in your region.
                </p>
